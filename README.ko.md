...
**Node.js 22 이상**이 필요합니다. Node.js 18 또는 20을 사용 중이라면 업데이트 전에 업그레이드하세요.

`apple_submit_for_review`에 `submissionId`를 지정하면 앱과 구독을 포함한 기존 심사 초안의 모든 항목을 제출합니다. 제출 응답 오류가 발생해도 자동 취소하지 않으며, 같은 ID로 재시도하면 이미 접수된 상태를 확인합니다.
...
로컬 clone이나 사전 설치가 필요하지 않습니다. 아래 MCP 클라이언트 설정이 `npx`로 npm 최신 버전을 실행합니다.
