...
**Node.js 22 以上**が必要です。Node.js 18・20 を使用している場合は、更新前にアップグレードしてください。

`apple_submit_for_review` に `submissionId` を指定すると、アプリと購読などを含む既存下書きの全項目を提出できます。提出の応答エラーでは自動取消を行わず、同じIDで再実行した際は受付済み状態を確認します。
